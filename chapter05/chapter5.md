...
结束了吗?不幸的是,并没有.上面的代码仍然着存在一些问题.我们在重复坐标来表示四边形.经过两次V2和V4坐标,有了这个小的外形,这看起来还没什么,但是想象一下一个更复杂的3D模型.我们会多次重复坐标.请记住,现在我们只是使用三个浮点数来表示顶点的位置.但稍后我们将需要更多的数据来表示纹理等.还要考虑到,在更复杂的形状中,三角形之间共享的顶点数可能会更多,如下图所示(其中一个顶点甚至可以在六个三角形之间共享):
